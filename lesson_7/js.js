...
const obstacle = {
	...
	render(obstaclePoint) {
		const obstacleCell = map.cells[`x${obstaclePoint.x}_y${obstaclePoint.y}`];
		obstacleCell.classList.add("obstacle");
		map.usedCells.push(obstacleCell);
	},
};
...
const game = {
	...
	tickHandler() {
		...
		} else if (this.obstacle.isOnPoint(this.snake.getNextStepHeadPoint())) {
			this.finish();
		}
	...
	getScore() {
		if (this.snake.getBody().length > this.score) {
			this.score = this.snake.getBody().length - 1;
			this.scoreTable.removeChild(document.querySelector(".score"));
			this.renderScore(this.score);
		}
	},

	renderScore(score) {
		this.scoreTable.insertAdjacentHTML(
			"afterbegin",
	...
	render() {
		...
		for (let i = 0; i < this.score; i++){
            this.obstacle.render(this.obstacle.getCoordinates());
        }
	},
};
